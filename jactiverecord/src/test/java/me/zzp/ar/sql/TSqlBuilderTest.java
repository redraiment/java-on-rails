...
public class TSqlBuilderTest {
  ...
  public void testHaving() {
    ...
    assertEquals("select sum(age) as sum, count(*) as count from users group by age having count(*) > 2", sql.toString());
  ...
  public void testQuery() {
    assertEquals("select age, count(*) from users where id > 1 and age >= 18 group by age having count(*) > 2 order by age desc limit 10 offset 100",
  ...
  public void testUpdate() {
    ...
    assertEquals("update users set name = ?, age = ? where id = ?", sql.update("users").set("name", "age").where("id = ?").toString());
    assertEquals("update users set age = ? where id > ? and name = ?", sql.update("users").set("age").where("id > ?", "name = ?").toString());
  }
  
  @Test
  public void testDelete() {
    assertEquals("delete from users", sql.delete().from("users").toString());
    assertEquals("delete from users where id > 2 and name = ?", sql.delete().from("users").where("id > 2", "name = ?").toString());
  }
